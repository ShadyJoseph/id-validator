...
class ApiKeyAdmin(admin.ModelAdmin):
    ...
    def display_generated_key(self, obj):
        ...
        return format_html(
            ...
            '<button type="button" onclick="hideKeyBox()" class="close-btn">X</button>'
            '<h3>Your API Key</h3>'
            ...
            '</div>'
            '<style>'
            '.key-display {{ background: #fff3cd; padding: 20px; border: 2px solid #ffc107; '
            'border-radius: 8px; margin: 10px 0; position: relative; }}'
            '.close-btn {{ position: absolute; top: 10px; right: 15px; background: none; '
            'border: none; font-size: 20px; cursor: pointer; color: #856404; }}'
            '.key-display h3 {{ color: #856404; margin: 0 20px 10px 0; }}'
            '.key-display p {{ color: #856404; margin: 10px 0; }}'
            '.api-key-code {{ display: block; background: #f8f9fa; padding: 10px; '
            'border: 1px solid #dee2e6; border-radius: 4px; margin: 10px 0; '
            'font-size: 14px; word-break: break-all; }}'
            '.button-group {{ margin-top: 15px; }}'
            '.copy-btn, .hide-btn {{ padding: 8px 16px; margin-right: 10px; border: none; '
